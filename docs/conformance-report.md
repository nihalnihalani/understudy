...
| §12 | Deployment — Fly manifests + Mac Mini launchd, both with cosign-verify pre-start | ✅ Matches | Fly manifest: `infra/fly/fly.toml` — 2 regions (iad, sjc) at `:37-41`, pre-start: `:29-30` `api = "/usr/local/bin/fly-start.sh"`, and `fly-start.sh:13-24` runs both `cosign verify` and `cosign verify-attestation --type slsaprovenance` before `exec python -m uvicorn`. Mac Mini launchd wrapper: `infra/fly/macmini-start.sh:33-46` runs both cosign commands, then `exec tinyfish serve`. Per-agent Fly template: `infra/fly/agent.fly.toml.tmpl` (exists, referenced by deployment.md). Plist: `infra/fly/macmini.plist`. | Both runtime paths gate on supply-chain verify. |
...
| SLSA L2 verify fails | Fly: `infra/fly/fly-start.sh:13-24`. Mac Mini: `infra/fly/macmini-start.sh:33-46`. Agent container: `infra/chainguard/agent/verify-self.sh:25-42`. Agent template peer: `apps/agent-template/src/preboot/verify.ts:37-59`. All four paths exit non-zero on fail. | ✅ |
...
| TinyFish Mac Mini browser pool | `infra/fly/macmini-start.sh:50-53` (`tinyfish serve --pool-size`). Mentioned in §12 diagram. | ✅ |
...
- **Code state:** The *in-app* switch works correctly (`gemini_client.py:32, 151-170`), but the *operational* script to flip DEMO_MODE on running Fly Machines + Mac Mini launchd is an `echo "TODO(task #9)"` stub at `scripts/demo_mode_switch.sh:12-13`.
- **Fix:** task #12: call `fly secrets set DEMO_MODE=$mode -a understudy-synthesis` + launchctl setenv on the Mac Mini, or document the manual procedure in `docs/demo-runbook.md`.
