...
| 2:15-2:30 | 15s | "Hit endpoint: agent runs via TinyFish CLI; TinyFish hosted browser session streamed on-screen; InsForge + Redis AMS fill live" | The core loop shells `tinyfish run --skill ... --script ...` (`apps/agent-template/src/tinyfish/cli.ts:45`). First hosted browser run: 5-20s depending on TinyFish pool warmth. Stream URL must be framed before the beat. | MEDIUM |
...
**Kill-switch state.** `scripts/demo_mode_switch.sh` only targets Fly.io + Docker Compose (we removed the Mac Mini launchd hop — Understudy doesn't operate a browser pool). Make sure `fly secrets set DEMO_MODE=replay -a understudy-synthesis` is memorized as the manual backup if the script hiccups.
...
| TinyFish hosted browser cloud | `apps/agent-template/src/tinyfish/cli.ts:45` shells out to `tinyfish run` → TinyFish hosted cloud | Code wired; TinyFish service availability on stage is a separate logistics question (Section 4 SPOF) | Yes if TinyFish cloud reachable |
...
| **500 agents per Fly.io host** | Architecture §9 note | **None.** No load test, no memory-footprint measurement. Just the int8 arithmetic implication. | "That's the implication of the int8 quantization arithmetic, not a measurement." |
...
| TinyFish hosted browser cloud unreachable | **Partial.** | If TinyFish's hosted API is down or our key is throttled, `tinyfish run` from `apps/agent-template/src/tinyfish/cli.ts:45` fails. The core loop bubbles the error to the GraphQL resolver. | **No fallback path.** There is no "mock TinyFish CLI" equivalent to `CosmoMockMCP`. If beat 2:15-2:30 fails because the TinyFish cloud 5xx's, we have no substitute. **Mitigation gap — propose adding a `TINYFISH_MOCK=1` env that returns a fixture browser-run result**, so beat 2:15-2:30 can fall back to a pre-recorded stream. |
...
**Second SPOF: no TinyFish mock.** Unlike Cosmo (which has `COSMO_MOCK=1`) and Gemini (which has `DEMO_MODE=replay`), TinyFish has no fallback. If the TinyFish hosted API is unreachable, beat 2:15-2:30 dies with no graceful degradation.
...
6. **Chainguard wolfi-base + skill pinning + pre-start cosign verify gate.** Real in Fly.io Machines and the agent verify-self container. Keep.
