...
- **SLSA L2 supply chain is non-negotiable.** Every generated agent image carries an SLSA L2 provenance predicate, a build-time SBOM, and a keyless cosign signature via Fulcio anchored in Rekor. Do not merge changes that skip signing or downgrade to post-build scans. Verification runs on boot via Fly pre-start hooks and Mac Mini launchd wrappers (architecture.md §6).
